package com.servlink.servlink.service;

import com.servlink.servlink.dto.request.ProfissionalRequest;
import com.servlink.servlink.dto.request.ProfissionalPerfilRequest;
import com.servlink.servlink.dto.response.ProfissionalResponse;
import org.springframework.data.domain.Page;

public interface ProfissionalService {

    ProfissionalResponse criar(ProfissionalRequest request);

    ProfissionalResponse criarOuObterProfissionalAtual();

    ProfissionalResponse obterProfissionalAtual();

    ProfissionalResponse atualizarProfissionalAtual(ProfissionalPerfilRequest request);

    Page<ProfissionalResponse> buscar(
            Long cidadeId,
            Long categoriaId,
            String q,
            String bairro,
            int pagina,
            int tamanho);

    ProfissionalResponse atualizarFotoAtual(org.springframework.web.multipart.MultipartFile file);
}
